...
function GenLink(streams) {
    ...
    if (AUTOQUALITY) {
        if (Hls.isSupported()) {
            ...
            if (QUALITY == '720') {
                return ApiTunime.link_file(STREAMS[720][0].src, STREAMS[480][0].src, STREAMS[360][0].src);
            } else if (QUALITY == '480') {
                return ApiTunime.link_file(undefined, STREAMS[480][0].src, STREAMS[360][0].src);
            } else {
                return ApiTunime.link_file(undefined, undefined, STREAMS[360][0].src);
            }
        }
    } else {
